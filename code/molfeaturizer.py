...
def predefined_mordred(smiles, desc_type="atom"):
    
    calc1 = mordred.Calculator()    
    
    if desc_type  == "atom-bond-ring": 
    ...
    for smile in smiles:
        ...
        mol=Chem.AddHs(mol)
        fp = list(calc1(mol).fill_missing(value=0.).values())
        fp = np.array(fp).astype(np.float32).reshape(-1)
        fps.append(fp)
    fps = np.array(fps)
    return fps
...
class RDKitFeaturizer(MolFeaturizer, ABC):
    ...
    def transform_single(self, molecule: str) -> Tuple[np.ndarray, bool]:
        mol = Chem.MolFromSmiles(molecule.strip(), self.sanitize, self.replacements)
...
class PhysChemFeaturizer(RDKitFeaturizer):
    ...
    def __init__(
        ...
        distributions_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'physchem_distributions.json'
        )
    ...
    def get_descriptor_subset(subset: str, subset_size: int) -> List[str]:
        ...
        elif subset == 'surface-fragment':
            return PhysChemFeaturizer.get_fragment_descriptor_subset()[:subset_size] + PhysChemFeaturizer.get_surface_descriptor_subset()[:subset_size] #+PhysChemFeaturizer.get_graph_descriptor_subset()[:subset_size] 
        else:
            raise ValueError(
...
class HashAPFPFeaturizer(RDKitFeaturizer):
    def __init__(
        self,
        nBits=1024
    ):
